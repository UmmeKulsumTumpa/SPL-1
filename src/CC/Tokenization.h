...
string keywordIdentifierNumberCheck( int l, int col, string &item )
{
    if ( item.size() == 0 ) {
        return "";
    }

    string keywords[50] = {     "abstract", "assert", 
                                "boolean", "break", "byte",
                                ...
                                "this", "throw", "throws", "try", "transient",
                                "void", "volatile",
                                "while"
                            };
    ...
    for ( int i = 0; i < totalLine; i++ ) {
        ...
        for ( int j = 0; j < lenPerLine; ) {
            if ( operatorCheck( perline[ i ].text[ j ] ) && operatorCheckdup( perline[ i ].text[ j + 1 ] ) ) {
                string toR = keywordIdentifierNumberCheck( i, j, item );
                take << toR;
